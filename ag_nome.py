...
class Individuo():
    ...
    def crossover(self, outro_individuo): #reaproveitar
    ...
    def mutacao(self, taxa_mutacao): #reaproveitar
        for i in range(len(self.cromossomo)):
...
class AlgoritmoGenetico():
    ...
    def visualizaGeracao(self):
        ...
        print(f"Geração: {self.populacao[0].geracao} - Cromossomo: {''.join(melhor.cromossomo)} - Fitness: {melhor.fitness}")
    ...
    def resolver(self, taxa_mutacao, numero_geracoes):
        ...
        while self.melhor_solucao.fitness != len(target):
        ...
        print(f"Melhor solução -> Geração: {self.melhor_solucao.geracao} - Fitness: {self.melhor_solucao.fitness} - Cromossomo: {self.melhor_solucao.cromossomo}")
...
if __name__=="__main__":
    # chars = "abcdefghijklmnopqrstuvxzyw "
    chars = "01"
    item = {
        0: {"valor": 10, "peso": 5}
    }
